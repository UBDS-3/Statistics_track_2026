...
subtitle: "BD[S]^3 · July 16, 2026"
author: "Adapted from Davide Risso by Helena Crowell"
format: 
    revealjs:
        theme: simple
        smaller: true
...
In 1855, the English physician John Snow made a map <br>
of cholera cases and identified spatial _clusters_ of cases:

![](img/Snow-cholera-map-1.jpg){width=50%}

## Clustering as a discovery tool

The proximity of dense clusters of cases to the Broadstreet <br>
pump pointed to its water as a possible culprit; clustering <br>
enabled him to infer the source of the cholera outbreak:

![](img/Snow-new.png){width=75%}

## Intuition of clustering

We try to group together **similar** observations. <br>
But what do we mean by similarity?
...
We can think of observations as points in a multi-dimensional 
space defined by the variables that we measure on them:

![https://xkcd.com/388/](img/fuck_grapefruit.png)

It is intuitive to think that by measuring the distance between 
object in such a space will provide a way to group them.
...
$x =  [x_{1} \, \ldots \, x_{p}]$ and $y =  [y_{1} \, \ldots \, y_{p}]$ 
in a $p$-dimensional space is the square root of the sum of squares 
of the differences in all coordinate directions:
...
For instance, we may be interested in clustering <br> 
DNA sequences (e.g., in evolutionary biology).

. . .

The simplest way to compute distances between <br>
strings is the _Hamming distance_ (aka Edit distance) <br>
= # differences / # edits needed to make them equal.
...
```{r echo=TRUE}
df <- read.csv("survey.csv", check.names=FALSE)
df <- mutate(df, 
    across(where(is.character), toupper),
    across(where(is.character), ~gsub("\\s", "", .x)))
df[df == ""] <- NA
head(df)
```
...
```{r fig.width=15, fig.height=6, echo=-1}
par(mfrow=c(1,5), mar=c(4,2,2,2))
xs <- names(which(sapply(df, is.character)))
for (x in xs) boxplot(df$`Age (years)` ~ df[[x]], main=x, xlab=NULL)
```

## Isolation Random Forests

Useful for outlier detection, easy to run, and pretty fast.
...
ns <- c(nrow(pcs), 1)
plot(qcs, asp=1, 
    lwd=rep.int(c(1, 2)^2, ns),
    col=rep.int(c("grey", "red"), ns)) 
```
...
irf <- isotree::isolation.forest(qcs)
summary(iso <- predict(irf, qcs))
```

:::: {.columns}
::: {.column}
```{r echo=FALSE, fig.width=6, fig.height=5}
hist(iso, n=100, ylim=c(0, 600),
    col="steelblue", border="white", 
    xlab=NULL, main="Outlier scores")
```
:::
::: {.column}
```{r echo=FALSE, fig.width=6, fig.height=5}
hist(iso, n=100, ylim=c(0, 6),
    col="steelblue", border="white", 
    xlab=NULL, main="Outlier scores")
```
:::
