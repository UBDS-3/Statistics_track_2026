...
Rendered output is written next to each `.qmd` (e.g. `labs/multivariate/multivariate.solved.html`), except `public`, which is written to `docs/<lab>/index.html` (see "Published pages" below). The `randomness` lab is always rendered as `default` only, and is skipped for `public` since it doesn't yet gate solutions behind a param. `exploratory` has no solutions to hide, so it renders the same content for every type.
...
automatically once its `.qmd` declares an `include_answers` param (see
`labs/multivariate/multivariate.qmd` for the pattern of gating solution
chunks with it) — no script or workflow changes needed, `render.sh --type
public` detects it. Currently `multivariate` (answers gated) and
`exploratory` (no solutions to hide) are published.
