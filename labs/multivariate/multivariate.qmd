...
8. In the lecture, we have seen another way to investigate correlations in the data. Use the `pheatmap` function in the package with the same name, to visualise all pairwise correlations of the results from the ten disciplines in the athletes data set.
...
"Answer 1: The panels on the diagonal show the distributions of each single variable. For the continuous variables length, width and height, the histogram is used. For the categorical variable sex, a barplot with the number of cases for each category. The off-diagonal panels show all $4\times 3 /2$ pairwise comparisons. Pairs of continuous variables are shown with scatterplots and correlation coefficients, pairs of a continuous and a categorical variable are show with stratified (or \"faceted\") histograms and boxplots." 
