...
#| echo: false
#| results: asis
cat("The panels on the diagonal show the distributions of each single variable. For the continuous variables length, width and height, the histogram is used. For the categorical variable sex, a barplot with the number of cases for each category is used. The off-diagonal panels show all $4\\times 3 /2$ pairwise comparisons. Pairs of continuous variables are shown with scatterplots and correlation coefficients, pairs of a continuous and a categorical variable are shown with stratified (or \"faceted\") histograms and boxplots.")
```
:::
