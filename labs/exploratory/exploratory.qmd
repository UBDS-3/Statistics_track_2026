...
Let's start by reading in the counts matrix. Two packages used here:

1.  *readr* for reading in the files
...
library(readr)
library(here)
counts <- read_delim(here("labs/exploratory/counts.txt.gz"))
counts
```
...
#| message: false
library(dplyr)
peaks <- peaks |> rename(perc_gc = percentage_gc_content)
dplyr::glimpse(peaks)
```
...
Look at the histograms for the numeric variable table. How would you describe the distribution of the different numeric columns? Do these distributions make sense? Note that our subset of the data contains just the first 1,000 regions on chromosome 1.
:::

We will also run `skim` on the column data, focusing on the numeric variables. We do this twice: once on the data after de-selecting the columns `assigned` and `peak_count`, and once just on those two columns. This is because the scale of those two columns is much larger than the other numberic columns, so it helps with printing to isolate them.

```{r}
coldata |> 
  select(where(is.numeric)) |>
  select(!c(assigned, peak_count)) |> 
  skim()
```
...
## Cumulative densities, histograms, ggplot2
...
The "QQ-plot" aligns the quantiles of the data with those of a Normal distribution (or another reference distribution). If the values fall roughly on a line, this means the data is roughly normally distributed. I don't personally use QQ-plots very often, but if you have to compare to a reference distribution for some reason, these are standard.
...
-   What could you say, in broad terms, about the first six samples in terms of these plots (pick whichever plots you want to focus on, e.g. from the colored densities and correlation matrix)?
...
## Advanced case study: chromatin accessibility

The following is a more advanced lab, which assumes some knowledge of genomics and computational biology. Feel free to stop here, or explore the rest. Ask TAs or instructors if you want more information, or you can also ask various AI chatbots, as this is a very well studied dataset with multiple papers and tutorials built upon it, and thematically examining chromatin accessibility across sample conditions (including immune stimulation) is a common genomics task.
...
Syntax note: in the above code chunk, I use `%>%` at one point instead of `|>`. This minor detail is because only `%>%` allows for the use of `.` nested within a function (here nested within `mutate`). For the most part `|>` and `%>%` are exchangeable, with `|>` slightly preferred as it is part of base R.
...
tab |> dplyr::slice(which.max(sum_pc1))
max_tile <- tiles |> dplyr::slice(which.max(tab$sum_pc1))
p |>
  filter_by_overlaps(max_tile)
```

## Advanced case study: bootstrapping genomic data
