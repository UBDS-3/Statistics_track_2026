...
title: Regression
date: "14 July 2026"
author: "Galton data: Robert Gentleman. Remainder: Helena L. Crowell & Wolfgang Huber; spider data analysis adapted from Rafael Irizarry & Michael Love" 
format:
  html:
    code-fold: false
    code-tools: true
...
In the lecture we considered the data set from Galton, where he provides data on 963 children from 205 families ranging from 1-15 adult children. 29 of the children have non-numeric heights and are excluded.

We will open the data set and do a little preprocessing in order
to repeat some of the analyses there.  But the data is much richer
and includes heights of mothers and daughters as well as fathers and sons. So
one easy extension is to repeat the analysis in the lecture on mothers and daughters.
Some families had more than one child, so parity and other features can be explored as well.
...
```{r scatterplot-run, fig.align = "center", fig.height = 3, fig.width = 3, out.width="40%", echo=TRUE} 
...
Now we want to consider resampling repeatedly.  The code below will do that.
`B` determines how many times you will repeat this, while `N` says how big of a sample to take.

Interesting questions are comparing the variability in the estimate if you change N and what
do you think might happen if you change B.
...
```{r boxplot-1-run, fig.align = "center", fig.height = 3, fig.width = 3, echo=TRUE} 
#| out-width: "60%"
...
```{r conditional-averages-follow-line, fig.align = "center", echo=TRUE, fig.height = 3, fig.width = 3, out.width="100%"} 
galton_heights |>  
...
## Some simple simulation models

Here you can explore the simple simulations from the lecture. You can change different things like the form of the relationship, the amount of variability in the random part and see how that affects the different estimates.
...
Here things are more complicated but you can see how we fit a non-linear model and that there are good methods for doing predictions. Again changing different parameters will affect the outputs.
...
# Colinearity and confounding: Boston housing data
...
- return to the interaction question
- see fit a model without the interaction and examine the parameter estimates
- then add the interaction - what happens?
- which model seems more appropriate and why?
- you could look at regression diagnostics as well - don't limit yourself - use a LLM if you can
...
# Blocking and pairing: Fitness data
...
# Contrasts and interactions: Spider legs data
...
```{r}
#| label: spider_interactions
#| fig.cap: "Diagram of the estimated coefficients in the linear model. As before, the teal-green arrow represents the Intercept, which fits the mean of the reference group (here, the pull samples for leg L1). The purple, pink, and yellow-green arrows represent differences between the three other leg groups and L1. The orange arrow represents the difference between the push and pull samples for all groups."
#| code-fold: true
#| code-summary: "Show the code"
...
```{r estimated_group_variables, fig.cap="Diagram of the estimated coefficients in the linear model, with each term representing the mean of a combination of type and leg.",echo=TRUE}
stripchart(split(spider$friction, spider$group),
           vertical=TRUE, pch=1, method="jitter", las=2, xlim=c(0,11), ylim=c(0,2))
cols <- hcl.colors(8, "Set 2")
abline(h=0)
